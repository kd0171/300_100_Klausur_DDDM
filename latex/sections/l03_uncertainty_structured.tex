...
\NoteSection{解説}
不確実性のモデル選択は、持っている情報の水準に依存する。すべての情報が既知なら deterministic、実現値の確率が分かるなら stochastic、実現範囲やシナリオは分かるが確率が分からないなら quasi-stochastic と考える。\par

stochastic では期待値、分散、確率分布、サンプリングを使う。quasi-stochastic では、最良/最悪ケース、シナリオ、区間、Hurwicz、minmax regret などを使う。\par

重要なのは、詳細なモデルが常に良いわけではない点である。データが少ない、相関が無視できない、分布仮定が不自然な場合、確率モデルより区間やシナリオの方が適切なことがある。\par

\NoteSection{試験対策ポイント}
deterministic, stochastic, quasi-stochastic を情報量の違いとして区別し、どのモデルをいつ使うか説明できること。\par

\NoteSection{過去問関連}
\ExamItem{WS22/23 Aufgabe 5 (6点)}{旅行時間の不確実性、確率的/準確率的モデル、ロバスト解の違いを問う問題。}{確率的モデルでは確率分布があり、期待値・分散・サンプリングを使って判断する。準確率的モデルでは確率が分からず、シナリオ、区間、最悪ケース、Hurwicz、regret などで不確実性を扱う。ロバスト解は、単一の平均ケースに最適化するのではなく、複数のあり得る状況で大きく破綻しにくい解である。}
\ExamItem{WS24/25 Aufgabe 3 (12点)}{stochastic と quasi-stochastic の違い、および各手法を説明する問題。}{stochastic は実現値の確率分布または確率が既知で、期待値最小化、Monte Carlo sampling、期待値-分散基準などを使える。quasi-stochastic は確率が未知で、シナリオ集合、区間、Hurwicz基準、minmax regret、最悪ケース最適化などを使う。判断の基準は、データ量、確率分布の妥当性、リスク回避度、解釈可能性である。}
\ExamItem{SS24 Aufgabe 2a (3点)}{Same-Day配送で考えられる不確実性のカテゴリを挙げる問題。}{需要不確実性として注文発生・量・場所、リソース不確実性としてドライバーの利用可能性・スキル・車両/バッテリー、環境不確実性として交通、駐車、天候、道路障害を挙げる。}
\ExamItem{WS23/24 Aufgabe 6 (4点)}{時間帯に依存しない旅行時間のばらつきを確率分布として近似する手順を問う問題。}{旅行時間観測を集め、ヒストグラムや密度を確認し、候補分布を選び、パラメータを推定する。可視化や適合度検定で妥当性を確認し、意思決定モデルではその分布からサンプリングするか、期待値・分散を使う。}
\ExamItem{SS24 Aufgabe 2b (3点)}{サービス時間分布の典型的な性質を説明する問題。}{サービス時間は負にならないため非負の分布である。短い時間に集中しつつ、例外的に長い作業があり右裾が長くなることが多い。平均値だけでなく分散、標準偏差、分位点がサービス品質や遅延リスクに影響する。}
\ExamItem{SS25 Exercise 4 (4点)}{指数分布から確率的遷移をサンプリングする手順を問う問題。}{指数分布のパラメータを設定し、乱数を用いて実現値を発生させる。その実現値を外生情報または遷移時間として状態遷移に入れる。サンプルを複数回生成することで、期待的な挙動や解の安定性を評価できる。}

\NoteSection{確認問題と解答}
\begin{enumerate}
\item \textbf{deterministic, stochastic, quasi-stochastic を区別せよ。}\\
deterministic は実現値が既知で不確実性をモデル化しない。stochastic は実現値に確率があり分布・期待値・分散を使う。quasi-stochastic は確率が不明で、シナリオや区間により可能な実現を表す。
\item \textbf{確率分布を使えない場合を説明せよ。}\\
データが観測できない、新しいサービスで履歴がない、道路工事などでシステム構造が変化した、観測数が少なすぎる、隣接道路の相関が強く独立分布の畳み込みが不自然な場合である。
\end{enumerate}
...
\NoteSection{解説}
確率変数 X は、まだ確定していない値を表す。実現値 x\_i は実際に起こり得る値であり、それぞれに確率 p\_i が対応する。連続分布では確率密度関数と積分で扱う。\par

期待値は平均的な中心を表し、分散・標準偏差はばらつきを表す。変動係数 CV は標準偏差を平均で割った相対的ばらつきで、平均が異なるリンク間の比較に使える。分位点は、95\%点や5\%点のようにリスク境界を作るのに使える。\par

分布フィッティングでは、理論分布を選び、パラメータを推定し、適合度を検査する。ただし、データが少ない場合や相関が強い場合は、きれいな分布が得られても意思決定上は危険な場合がある。\par

\NoteSection{試験対策ポイント}
確率変数、期待値、分散、標準偏差、変動係数、分位点、フィッティング、サンプリングをそれぞれ説明できること。\par
...
\NoteSection{確認問題と解答}
\begin{enumerate}
\item \textbf{期待値、分散、標準偏差、変動係数を説明せよ。}\\
期待値は長期平均または中心的な値である。分散は値が期待値からどれだけ散らばるかを表す。標準偏差は分散の平方根で元の単位に戻したばらつきである。変動係数は標準偏差を平均で割った相対的ばらつきで、平均値が異なる対象同士のリスク比較に使いやすい。
\item \textbf{分布フィッティングの手順を説明せよ。}\\
候補となる理論分布を選び、観測データから平均・分散などのパラメータを推定し、ヒストグラムやQQプロット、Kolmogorov-Smirnov検定などで適合度を確認する。必要に応じてブートストラップでパラメータ推定の安定性を検査する。
\item \textbf{Sampling は意思決定で何に使うか。}\\
分布からランダムな実現値を生成し、多数のシナリオを作るために使う。これにより期待コストや解の安定性を近似的に評価できる。
\end{enumerate}
...
\NoteSection{解説}
stochastic decision making では、決定 x を選んだ後にランダム情報 xi が実現する。したがって F(x, xi) を実現前に直接最小化することはできず、期待値 E[F(x, xi)] やリスクを含む評価関数を最小化する。\par

Monte Carlo Sampling では、xi のサンプルを n 個生成し、期待値を標本平均で近似する。これにより確率的目的関数を、有限個のシナリオに基づく決定論的な評価へ変換できる。\par

Expected Value Variance Principle は、平均だけでなく分散も評価に入れる。リスク回避的ならばばらつきを嫌い、リスク志向ならば高いばらつきを受け入れる。\par

\NoteSection{試験対策ポイント}
決定 x とランダム情報 xi の順序、期待値最小化、Monte Carlo近似、期待値-分散基準を説明できること。\par
...
\NoteSection{確認問題と解答}
\begin{enumerate}
\item \textbf{なぜ F(x, xi) を直接最適化できないのか。}\\
xi は決定後に分かるランダム情報だからである。意思決定時点ではどの実現値になるか分からないため、期待値、標本平均、分散を含む基準など、事前に評価可能な関数に変換する必要がある。
\item \textbf{Monte Carlo Sampling による近似を説明せよ。}\\
ランダム情報 xi のサンプルを n 個生成し、それぞれについて F(x, xi) を評価する。期待値 E[F(x, xi)] を 1/n sum\_i F(x, xi\_i) で近似し、その近似目的関数を最小化する。
\item \textbf{期待値-分散基準で q の意味を説明せよ。}\\
q は分散をどれだけ重視するかを示す重みである。q=0なら期待値のみを見る。q>0ならばばらつきをコストとして嫌うリスク回避的判断になる。q<0ならばばらつきを好むリスク志向的判断になる。
\end{enumerate}
...
\NoteSection{解説}
quasi-stochastic decision making では、確率分布が不明であるため、期待値計算ではなくシナリオ、区間、最悪/最良ケースを使う。確率がないため、どの実現がどれくらい起こりやすいかは直接評価しない。\par

Hurwicz基準は、最悪ケースと最良ケースを楽観係数 lambda で重み付けする。lambda が大きいほど楽観的で、最良ケースを重視する。lambda が小さいほど保守的で、最悪ケースを重視する。\par

Minmax Regret は、各シナリオで完全情報があれば選べた最良解との差を regret として測る。各候補解の最大 regret を比較し、その最大 regret が最小になる解を選ぶ。\par

\NoteSection{試験対策ポイント}
Hurwiczとminmax regretを計算基準として説明し、確率分布がない場合の判断方法として位置付けること。\par

\NoteSection{過去問関連}
\ExamItem{SS23 Aufgabe 3 (6点)}{区間旅行時間に基づく minmax regret の考え方を問う問題。}{各シナリオまたは区間実現で、完全情報を持つ最良解のコストと選んだ解のコストとの差を regret とする。各解について最大 regret を計算し、その最大値が最小になる解を選ぶ。平均に最適化するのではなく、後から『別の解にしておけばよかった』という最悪の後悔を抑える。}
\ExamItem{WS24/25 Aufgabe 1 (10点)}{楽観/悲観旅行時間、複数値、regret を用いたツアープランニングを問う問題。}{楽観値だけでは実行不能・遅延リスクが高く、悲観値だけでは過度に保守的になる。複数シナリオや区間を使って候補ルートの性能を評価し、regret や Hurwicz により効率性と信頼性のバランスを取る。}
\ExamItem{SS25 Exercise 8 (4点)}{Hurwicz基準の機能と適用条件を問う問題。}{Hurwicz基準は、最悪ケースと最良ケースを楽観係数 lambda で重み付けする準確率的判断基準である。確率分布は分からないが、最良・最悪の境界やシナリオは分かる場合に適用できる。lambda が大きいほど楽観的、lambda が小さいほど保守的である。}
\ExamItem{WS22/23 Aufgabe 5 (6点)}{旅行時間の不確実性、確率的/準確率的モデル、ロバスト解の違いを問う問題。}{確率的モデルでは確率分布があり、期待値・分散・サンプリングを使って判断する。準確率的モデルでは確率が分からず、シナリオ、区間、最悪ケース、Hurwicz、regret などで不確実性を扱う。ロバスト解は、単一の平均ケースに最適化するのではなく、複数のあり得る状況で大きく破綻しにくい解である。}
\ExamItem{WS24/25 Aufgabe 3 (12点)}{stochastic と quasi-stochastic の違い、および各手法を説明する問題。}{stochastic は実現値の確率分布または確率が既知で、期待値最小化、Monte Carlo sampling、期待値-分散基準などを使える。quasi-stochastic は確率が未知で、シナリオ集合、区間、Hurwicz基準、minmax regret、最悪ケース最適化などを使う。判断の基準は、データ量、確率分布の妥当性、リスク回避度、解釈可能性である。}
\ExamItem{SS24 Aufgabe 2a (3点)}{Same-Day配送で考えられる不確実性のカテゴリを挙げる問題。}{需要不確実性として注文発生・量・場所、リソース不確実性としてドライバーの利用可能性・スキル・車両/バッテリー、環境不確実性として交通、駐車、天候、道路障害を挙げる。}
...
\NoteSection{確認問題と解答}
\begin{enumerate}
\item \textbf{Hurwicz基準を説明せよ。}\\
最悪ケースの値と最良ケースの値を、楽観係数 lambda で重み付けして一つの評価値にする。最大化なら (1-lambda)×worst + lambda×best を最大化する。lambda は意思決定者の楽観度を表す。
\item \textbf{Minmax regret を説明せよ。}\\
あるシナリオで、選んだ解の性能と、そのシナリオを完全に知っていた場合の最良性能との差を regret とする。各候補解について最悪の regret を求め、その最大 regret が最小の解を選ぶ。
\item \textbf{Hurwicz と minmax regret の違いを説明せよ。}\\
Hurwicz は各解の最良値と最悪値を重み付けする。minmax regret は各シナリオでの完全情報最適解との差、つまり後悔を評価する。Hurwiczは楽観度、minmax regretは機会損失の抑制に焦点を置く。
\end{enumerate}
...
\NoteSection{解説}
都市物流では、交通混雑や旅行時間の変動により、平均的に短いルートが実際には遅延しやすいことがある。したがって効率性、つまり旅行時間の短さと、信頼性、つまり予定時刻に間に合うことを同時に考える必要がある。\par

区間旅行時間モデルでは、各顧客間旅行時間を下限 L と上限 U の二つの行列で表す。例えば5\%分位点を下限、95\%分位点を上限とすれば、楽観的・悲観的な実現範囲を簡潔に表現できる。\par

Robust TSP/VRP では、単純な最良ケースや平均ケースではなく、複数の交通速度シナリオで実装可能で、遅刻・早着の偏差を抑えるルートを目指す。regret based approach は、効率性とサービス信頼性のバランスを取るための考え方である。\par

\NoteSection{試験対策ポイント}
区間旅行時間に基づくロバストルーティングで、効率性と信頼性のトレードオフを説明できること。\par
...
\NoteSection{確認問題と解答}
\begin{enumerate}
\item \textbf{区間旅行時間モデルを説明せよ。}\\
各アークまたは顧客間について、旅行時間を一つの値ではなく [l\_ij, u\_ij] で表す。l\_ij は楽観的または下限の旅行時間、u\_ij は悲観的または上限の旅行時間である。確率分布を仮定せずに不確実性の幅を表せる。
\item \textbf{なぜ最良ケースだけで計画してはいけないか。}\\
最良ケースは交通が非常に良い場合を前提にするため、現実に少しでも混雑すれば遅延し、顧客のサービス時刻や同期制約を破る可能性が高い。
\item \textbf{平均ケースと最悪ケースの問題点を説明せよ。}\\
平均ケースは信頼性を十分保証しない可能性がある。最悪ケースは安全だが過度に保守的で、旅行時間や車両数が増え効率が悪くなる。robust/regret型はこの中間を狙う。
\end{enumerate}
